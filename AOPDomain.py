...
class AOPDomain(object):
    ...
    #    def _overlap(self, option: str, return_type: Optional[str] = None):
    #        '''
    #        subroutine to calculate gap fraction (Pvg & Pig), Pv|Ps, Pvc|Psc
    #        @param option: option to determine what kind of returns
    #        @param return_type: request for return type, default: None
    ...
    #                ii = i * 1.
    #                jj = j * 1.
    #                if ptj[i - 1] < 0.0000001 and ii > i_0:
    #                    ptj[i] = 0
    #                    ptjc[i] = 0
    #                else:
    #                    aa = a
    ...
    #                        ptj[i] = 0
    #                        if ii != 0:
    #                            ptj[i] = px[i] * pow(1. - aa / A, ii)
    #                        ptjc[i] = 0
    #                        if ii != 0:
    #                            ptjc[i] = px[i] * pow(1. - aac / A, ii)
    #                    else:
    #                        pnn = px[i]
    #                        pnnc = px[i]
    #                        if aa != A:
    #                            pnn = px[i] * pow(1. - aa / A, ii - jj)
    #                        if aac != A:
    #                            pnnc = px[i] * pow(1. - aac / A, ii - jj)
    ...
    #                return ptree
    #            else:
    #                raise ValueError('Wrong request of return type under current option SZA: it must be Pig, Ps or Psc')
    #        elif 'LAI' == option.upper():
    #            if return_type is None or 'Pvg_mean' == return_type:
    #                return p
    #            else:
    ...
    def compare(self):
        omega_total = self.OmegaTotal
        cv = self.Cv
        LAI = self.domain.tree.LAI
        delta_LAI = self.domain.tree.DeltaLAI

        def f(x):
            arg4 = 0.5 * omega_total * x / cos(asin(0.537 + 0.025 * x))
            return 0.5 * exp(-arg4) * exp(-cv * x)

        start1 = time.time()
        v, err = integrate.quad(f, 0, LAI)
        end1 = time.time()
        print('scipy: %f' % (end1 - start1))
        print('scipy value: %f' % v)
        ...
        while li < LAI:
            ...
            li = li + delta_LAI
        end2 = time.time()
        print('while:%f' % (end2 - start2))
        print('while value:%f' % f_s_t)

        print('Is scipy faster than while? The answer is %r' % (end1 - end2 + start2 - start1 < 0))


# option = ['LAI', 'NADIR']
## vza_type = ['Pvg', 'Pvc', 'Pv']
## sza_type = ['Pig', 'Psc', 'Ps']
# vza_type = ['Pvg', 'Pvc', 'Pv', 'Ptreev']
# sza_type = ['Pig', 'Psc', 'Ps', 'Ptrees']
# for t in vza_type:
#    start1 = time.time()
#    value1 = self._overlap('VZA', t)
#    end1 = time.time()
#    print('execution time of overlap: %.6f' % (end1 - start1))
#    start2 = time.time()
#    value2 = self._overlap_v1('VZA', t)
#    end2 = time.time()
#    print('execution time of overlap_v2: %.6f' % (end2 - start2))
#    # if 'Ptreev' != t:
#    #    out_str = 'VZA %s: \nvalue1:%.2f, value2:%.2f\n' % (t, value1, value2)
#    #    print(out_str)
#    # else:
...
#    #    value2 = value2.reshape([value2.size, 1])
#    #    value = np.concatenate((value1, value2), axis=1)
#    #    print(value)

# for t in sza_type:
#    value1 = self._overlap('SZA', t)
#    value2 = self._overlap_v1('SZA', t)
#    if 'Ptrees' != t:
#        out_str = 'VZA %s: \nvalue1:%.2f, value2:%.2f\n' % (t, value1, value2)
#        print(out_str)
#    else:
...
#        print(value)
# for t in option:
#    value1 = self._overlap(t)
#    value2 = self._overlap_v1(t)
#    out_str = '%s: \nvalue1:%.2f, value2:%.2f\n' % (t, value1, value2)
#    print(out_str)
