...
class GeoVI(object):
    ...
    def __init__(self, SZA: float, VZA: float, phi: float):
        ...
        self.__VZA = VZA
        self.__phi = phi
        self.__xi = GeoVI.XI(self.SZA, self.VZA, self.phi)
    ...
    def SZA(self, value: float):
        self.__init__(value, self.__VZA, self.__phi)
    ...
    def VZA(self, value: float):
        self.__init__(self.__SZA, value, self.__phi)
    ...
    def phi(self, value: float):
        self.__init__(self.__SZA, self.__VZA, value)
...
class AOPConeTree(AOPTree, AOPSpecies):
    ...
    def __init__(self, tree: ConeTree, geovi: GeoVI):
        ...
        self.__Tab, self.__Tac = self._ta()
        self.__Vg_0, self.__Vgc = self._gs('VZA')
        self.__Vg_0_mean, self.__Vgc = self._gs('LAI')
        self.__Sg_0, self.__Sgc = self._gs('SZA')
        xi = self.geovi.xi
        if 0.000001 >= xi >= 0:
            self.__Tib = self.tab
            self.__Tic = self.tac
        else:
            self.__Tib, self.__Tic = self._ti()
        SZA = self.geovi.SZA
        if SZA == pi or SZA == 0:
            h = 0
        else:
        ...
        V = self.tree.V
        VZA = self.geovi.VZA
        self.__Sv = V / (self.__Vg_0 * cos(VZA))
        self.__Ss = V / (self.__Sg_0 * cos(SZA))
...
class AOPSpheroidTree(AOPTree, AOPSpecies):
    ...
    def __init__(self, tree: SpheroidTree, geovi: GeoVI):
        AOPTree.__init__(self, tree, geovi)
        self.__Tab = self._ta()
        self.__Vg_0 = self._gs('VZA')
        self.__Vg_0_mean = self._gs('LAI')
        self.__Sg_0 = self._gs('SZA')
        xi = self.geovi.xi
        if 0.000001 >= xi >= 0:
            self.__Tib = self.tab
        else:
        ...
        Vg_0 = self.Vg_0
        VZA = self.geovi.VZA
        self.__Sv = V / (Vg_0 * cos(VZA))
        Sg_0 = self.Sg_0
        SZA = self.geovi.SZA
        self.__Ss = V / (Sg_0 * cos(SZA))
        if SZA == pi or SZA == 0:
            self.__H = 0
        else:
            Ha = self.tree.Ha
